...
class Xsdk(BundlePackage):
    ...
    depends_on('mfem@4.2.0-xsdk+mpi+superlu-dist+petsc~sundials+examples+miniapps', when='@0.6.0') #sundials
    ...
    depends_on('trilinos@13.0.0+hypre+superlu-dist+metis+hdf5~mumps+boost~suite-sparse+tpetra+nox+ifpack2+zoltan2+amesos2~exodus~dtk+intrepid2+shards gotype=int',
               when='@0.6.0 +trilinos')
    ...
    depends_on('sundials@develop~int64+hypre+petsc+superlu-dist+trilinos', when='@develop')
    depends_on('sundials@5.4.0~int64+hypre+petsc+superlu-dist+trilinos', when='@0.6.0')
    ...
    depends_on('pumi@2.2.4', when='@0.6.0')
    ...
    depends_on('phist@1.7.5 ~fortran ~scamac ~host ~int64', when='@0.4.0 +phist')
